...
def main():
	...
	for item in player.find('cookingRecipes').findall('item'):
		known_recipes.add(item.find('key').find('string').text)
	if len(cooked_recipes) < len(recipe_ids.keys()):
		...
		missing = recipe_ids.keys() - cooked_recipes
